...
impl AppState {
    ...
    pub fn aimbot_settings(&mut self, ui: &mut Ui) {
        ui.horizontal(|ui| {
            ui.selectable_value(&mut self.aim_feature_tab, AimFeatureTab::Aim, "Aim Assist");
            ui.selectable_value(
                &mut self.aim_feature_tab,
                AimFeatureTab::Trigger,
                "Triggerbot",
            );
        });
        ui.add_space(4.0);
        ...
        match self.aim_feature_tab {
            AimFeatureTab::Aim => scroll(ui, "aim_main", |ui| self.aim_main(ui)),
            AimFeatureTab::Trigger => scroll(ui, "trigger_main", |ui| self.trigger_main(ui)),
        };
