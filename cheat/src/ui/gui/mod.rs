use egui::{Align, Ui};
use shared::{Data, SettingId};

use crate::{
    config::{
        aim::WeaponConfig,
        bind::{BindMode, KeyChord, SettingBind},
        write_config,
    },
...
impl AppState {
    ...
    pub fn bool_setting_hover(
        ...
        if response.open_bind {
            if self.bind_popup.is_none() {
                self.send_message_game(GameMessage::BindCapture(true));
            }
    ...
    fn render_bind_popup(&mut self, ui: &mut Ui) {
        ...
        if !open || close {
            ...
            self.send_message_game(GameMessage::BindCapture(false));
        }
    }
